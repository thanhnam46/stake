...
function Stake(props) {
  ...
  async function unStakeToken() {
    ...
    if (amount === '' || amount < 0) {
      ...
      await props.stakingContract.methods.withdraw(amount).send({ from: props.account })
  ...
  return (
    <div className='content'>
      {messageVisibility && <MessageBoard message={message} txHash={txHash} error={error} clsBtnVis={clsBtnVis} />}

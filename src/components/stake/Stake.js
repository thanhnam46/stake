...
function Stake(props) {
    ...
    async function unStakeToken() {
        ...
        if (Date.now() < props.earlyWithdraw * 1000) {
          ...
          If you still want to do the early withdraw, please confirm it on your Wallet`,
                ...
                setMessageVisibility(true);
                setMessage(`
          You will get ~${MaturityReward(props.rewardState.rewardBalance, amount, props.stakedTotal)} SPO as the reward.
    ...
    return (
        <div className="content">
            {messageVisibility && (
                <MessageBoard message={message} txHash={txHash} error={error} clsBtnVis={clsBtnVis} />
            )}
            <div className="stakeBox">
                ...
                <p>
                    {(props.chain == process.env.REACT_APP_NETWORK_VERSION) |
                    (props.chain == process.env.REACT_APP_NETWORK_VERSION_HEX) ? (
                        'You are connected to BSC'
                    ) : (
